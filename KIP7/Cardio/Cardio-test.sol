...
contract KIP7 is KIP13, IKIP7, Ownable {
    ...
    uint256 public timeController = 0;
    ...
    function setTimeController(uint256 time) public {
        timeController = time;
    }
    ...
    function _unLock(address sender, uint8 tokenType) internal {
        ...
        if(tokenType == 1 && _exchangeListingTime <= timeController && lockInfo.remainUnLockCount == 5) {
            // lockInfo update
            lockInfo.distributedTime = _exchangeListingTime;
            lockInfo.lastUnlockTimestamp = timeController;
        ...
        if(_isOverLockUpPeriodMonth((timeController.safeSub(lockInfo.distributedTime)), lockInfo.lockUpPeriodMonth) == false) {
            return;
        }

        uint256 blockTime = timeController;
